        ...
        3: [function(require, module, exports) {
            ...
            GIF = function(superClass) {
                ...
                GIF.prototype.spawnWorkers = function() {
                    var j, numWorkers, ref, results,k = 0;
                    ...
                    (function() {
                        ...
                        return function(i) {
                            ...
                            worker = new Worker(_this.options.workerScript, { name : 'myWorker'+ k++ });
                            worker.onmessage = function(event) {
								//worker 线程处理完成返回的数据，activeWorkers 减少一个worker
                                _this.activeWorkers.splice(_this.activeWorkers.indexOf(worker), 1);
								// 空闲 worker 放进freeWorkers中
                ...
                GIF.prototype.finishRendering = function() {
					...
					//ArrayBuffer对象表示原始的二进制数据缓冲区，即在内存中分配指定大小的二进制缓冲区（容器），
					//用于存储各种类型化数组的数据，是最基础的原始数据容器，无法直接读取或写入，
					//需要通过具体视图来读取或写入，即TypedArray对象或DataView对象对内存大小进行读取或写入；
					...
					// image.arrayBuffer() = 》 Promise 
					image.arrayBuffer().then(function(y){
						console.log(y)
					})
                    image = new Blob([data],{
                        type: "image/gif"
                ...
                GIF.prototype.renderNextFrame = function() {
                    ...
                    worker = this.freeWorkers.shift();// 从 freeWorkers拿出一个
                    task = this.getTask(frame);
                    this.log("starting frame " + (task.index + 1) + " of " + this.frames.length);
                    this.activeWorkers.push(worker); // 放进 activeWorkers 增加一个
					// worker 拿出来，task拿出来放到worker线程中执行，
